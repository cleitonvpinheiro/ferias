...
const cpfInput = form ? form.querySelector('input[name="cpf"]') : null;
if (cpfInput) {
...
// Lógica de Indicação
function toggleIndicacao(valor) {
    const campo = document.getElementById('campoIndicacao');
...
function limparIndicacao() {
    ...
    document.getElementById('indicadoPorNome').value = '';
    document.getElementById('indicadoPorCargo').value = '';
    document.getElementById('indicadoPorSetor').value = '';
    document.getElementById('infoIndicacao').style.display = 'none';
...
if (inputBusca) {
    ...
    const termoAtual = termo;
    try {
        const res = await fetch(`/api/public/funcionarios/busca?busca=${encodeURIComponent(termoAtual)}`);
        ...
        if (inputBusca.value.trim() === termoAtual) {
            renderSugestoes(funcionarios);
        }
    } catch (err) {
...
function renderSugestoes(lista) {
    ...
    lista.forEach(f => {
        ...
        item.innerHTML = `<strong>${f.nome}</strong><br><small>${f.cargo} - ${f.setor}</small>`;
        item.onmouseover = () => item.style.background = '#f3f4f6';
        item.onmouseout = () => item.style.background = 'white';
        item.onclick = () => selecionarColaborador(f);
...
function selecionarColaborador(f) {
    ultimoTermo = f.nome;

    document.getElementById('buscaColaborador').value = f.nome;
    document.getElementById('indicadoPorNome').value  = f.nome;
    document.getElementById('indicadoPorCargo').value = f.cargo;
    document.getElementById('indicadoPorSetor').value = f.setor;

    const info = document.getElementById('infoIndicacao');
    info.innerHTML = `<strong>Colaborador selecionado:</strong><br>${f.nome}<br>${f.cargo} | ${f.setor}`;
...
document.addEventListener('click', (e) => {
    if (!e.target.closest('#campoIndicacao')) {
        document.getElementById('listaSugestoes').style.display = 'none';
    }
});
...
form.addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (cpfInput) {
            const cpfDigits = normalizeCpfDigits(cpfInput.value);
            if (cpfDigits) {

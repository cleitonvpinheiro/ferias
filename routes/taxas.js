...
const responderAprovacaoLimiter = rateLimit({
    windowMs: 10 * 60 * 1000,
    max: 120,
...
module.exports = (_db, auth) => {
    ...
    const taxasFormAuth = [verifyToken, checkRole([ROLES.DP, ROLES.RH_GERAL, ROLES.RH, ROLES.GESTOR, ROLES.LIDER, ROLES.SUPERVISOR, ROLES.GERENTE])];
    const gestorTaxasAuth = [verifyToken, checkRole([ROLES.GESTOR, ROLES.LIDER, ROLES.SUPERVISOR, ROLES.GERENTE])];
...
router.post('/taxas', taxasFormAuth, async (req, res) => {
    try {
        ...
        if (req.user && [ROLES.GESTOR, ROLES.LIDER, ROLES.SUPERVISOR, ROLES.GERENTE].includes(req.user.role)) {
            ...
            if (!depNorm || !setores.includes(depNorm)) {
                return res.status(403).json({ message: 'Departamento inválido para este perfil.' });
            }
...
router.get('/rh/beneficios/ultimas-compras', dpAuth, async (req, res) => {
    try {
        ...
        // Processa solicitações de taxas aprovadas
        ...
        // Processa registros de benefícios importados
        extras.forEach(item => {
            const key = norm(item.cpf);
            if (alvoCpf && key !== alvoCpf) return;
            const quandoDate = item.data_referencia ? new Date(item.data_referencia) : null;
            ...
            if (tipo === 'VT' || tipo === 'VALE TRANSPORTE') {
                const currentDate = map[key].ultimoVT ? new Date(map[key].ultimoVT) : null;
                if (!currentDate || (quandoDate && quandoDate > currentDate)) {
                    map[key].ultimoVT = quandoDate ? quandoDate.toISOString() : null;
                    map[key].ultimoVTValor = Number(item.valor || 0);
                }
            } else {
                const currentDate2 = map[key].ultimoOutro ? new Date(map[key].ultimoOutro) : null;
                if (!currentDate2 || (quandoDate && quandoDate > currentDate2)) {
                    map[key].ultimoOutro = quandoDate ? quandoDate.toISOString() : null;
                    map[key].ultimoOutroDescricao = item.descricao || item.tipo;
                }
            }
        });
...
// Importação de benefícios via Excel
router.post('/rh/beneficios/importar', dpAuth, async (req, res) => {
    try {
        ...
        const criado_por = req.user && (req.user.name || req.user.username) ? req.user.name || req.user.username : 'Importação Excel';

        let importados = 0;
        for (const item of items) {
            const cpf = String(item.cpf || item.CPF || '').replace(/\D/g, '');
            if (!cpf) continue;

            const id = crypto.randomUUID();
            const data_ref = item.data || item.data_referencia || item['Data Referência'] || item['Data'];
            
            const payload = {
                id,
                cpf,
                tipo: String(item.tipo || item.Tipo || 'VT').trim(),
                valor: parseFloat(String(item.valor || item.Valor || 0).replace(',', '.')),
                data_referencia: data_ref ? new Date(data_ref).toISOString().slice(0, 10) : now.slice(0, 10),
                descricao: String(item.descricao || item.Descrição || '').trim(),
                criado_por,
                created_at: now
            };

            await db.beneficios.create(payload);
            importados++;
        }

        res.json({ ok: true, importados });
    } catch (e) {
        console.error('Erro na importação de benefícios:', e);
        res.status(500).json({ ok: false, erro: 'Erro ao importar benefícios' });
    }
});
...
router.get('/taxas/responder-aprovacao', async (req, res) => {
    try {
        ...
        const titulo = acao === 'aprovar' ? 'Confirmar aprovação' : 'Confirmar reprovação';
        const cor = acao === 'aprovar' ? '#16a34a' : '#dc2626';
        const label = acao === 'aprovar' ? 'Aprovar' : 'Reprovar';
        const nome = escapeHtml(item.nome_taxa || item.nomeTaxa || item.nome || 'Solicitação');
        ...
        res.send(`
            <html>
                ...
                <body style="font-family: sans-serif; text-align: center; padding: 50px;">
                    <div style="max-width: 520px; margin: 0 auto; text-align: left; border: 1px solid #e5e7eb; border-radius: 14px; padding: 18px;">
                        <h1 style="margin: 0 0 10px; color: ${cor}; font-size: 22px;">${titulo}</h1>
                        <p style="margin: 0 0 14px; color: #374151; line-height: 1.4;">${nome}</p>
                        <form method="POST" action="/api/taxas/responder-aprovacao">
                            <input type="hidden" name="token" value="${escapeHtml(token)}" />
                            <input type="hidden" name="acao" value="${escapeHtml(acao)}" />
                            <button type="submit" style="width: 100%; padding: 12px 14px; border: 0; border-radius: 12px; background: ${cor}; color: white; font-weight: 800; cursor: pointer;">${label}</button>
                        </form>
                        <p style="margin: 12px 0 0; color: #6b7280; font-size: 13px;">Após confirmar, você pode fechar esta janela.</p>
                    </div>
                </body>
            </html>
        `);
...
router.post('/taxas/responder-aprovacao', responderAprovacaoLimiter, async (req, res) => {
    try {
        const token = req.body && typeof req.body.token === 'string' ? req.body.token : (req.query && req.query.token);
        const acao = req.body && typeof req.body.acao === 'string' ? req.body.acao : (req.query && req.query.acao);
        if (!token || !['aprovar', 'reprovar'].includes(acao)) {
            return res.status(400).send('Link inválido.');
        }
        ...
        const cor = acao === 'aprovar' ? 'green' : 'red';
        res.send(`
            <html>
                <head>
                    <meta charset="UTF-8" />

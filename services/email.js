...
async function notificarGestorVaga(vaga, status, justificativa) {
    ...
    const user = process.env.SMTP_USER;
    const pass = process.env.SMTP_PASS;
    const from = process.env.MAIL_FROM || process.env.SMTP_FROM || '"RH Madalosso" <rh@familiamadalosso.com.br>';
...
async function enviarEmailAlertaGestor({ to, subject, text }) {
    ...
    const pass = process.env.SMTP_PASS;
    const secure = String(process.env.SMTP_SECURE || 'false') === 'true';
    const from = process.env.MAIL_FROM || process.env.SMTP_FROM || '"Portal RH" <no-reply@portal.local>';

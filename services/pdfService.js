...
function pdfBufferFromAvaliacaoCicloRelatorio(payload) {
    const doc = new PDFDocument({ size: 'A4', margin: 40 });
    const chunks = [];
    doc.on('data', c => chunks.push(c));
    return new Promise(resolve => {
        ...
        const titulo = 'RELATÓRIO FINAL - CICLO DE AVALIAÇÃO';
        doc.rect(40, 40, doc.page.width - 80, 50).fillAndStroke('#333333', '#000000');
        doc.fillColor('#FFFFFF').fontSize(16).text(titulo, 40, 58, { width: doc.page.width - 80, align: 'center' });
        doc.fillColor('#000000').fontSize(10);
        ...
        const ciclo = payload && payload.ciclo ? payload.ciclo : {};
        const avaliado = payload && payload.avaliado ? payload.avaliado : {};
        const cons = payload && payload.consolidado ? payload.consolidado : {};

        const pct = cons && cons.mediaPct != null ? `${(Number(cons.mediaPct) * 100).toFixed(1)}%` : '-';
        const concluidas = `${Number(cons && cons.concluidas || 0)}/${Number(cons && cons.totalSolicitadas || 0)}`;
        ...
        const ensureSpace = (min) => {
            if (y + min <= doc.page.height - 50) return;
            doc.addPage();
            y = 50;
        };

        textRow('Ciclo', ciclo.titulo || '');
        textRow('Status', ciclo.status || '');
        textRow('Tipo', ciclo.tipo_formulario || '');
        textRow('Modelo', ciclo.modelo || '');
        y += 6;
        line();

        doc.font('Helvetica-Bold').fontSize(12).text('Colaborador', 40, y);
        doc.fontSize(10);
        y += 18;
        textRow('Nome', avaliado.nome || '');
        textRow('Setor', avaliado.setor || '');
        textRow('Cargo', avaliado.cargo || '');
        y += 6;
        line();

        doc.font('Helvetica-Bold').fontSize(12).text('Resumo', 40, y);
        doc.fontSize(10);
        y += 18;
        textRow('Média final (ponderada)', pct);
        textRow('Concluídas', concluidas);
        y += 6;
        line();

        const drawTable = ({ title, columns, rows, colWidths }) => {
            ensureSpace(80);
            doc.font('Helvetica-Bold').fontSize(12).text(title, 40, y);
            doc.fontSize(10);
            y += 18;
            ...
            const headerH = 18;
            const rowH = 18;
            const widths = Array.isArray(colWidths) && colWidths.length === columns.length
                ? colWidths
                : columns.map(() => Math.floor(wFull / columns.length));

            doc.rect(tableX, y, wFull, headerH).fillAndStroke('#e5e7eb', '#000000');
            doc.fillColor('#000000').font('Helvetica-Bold');
            let x = tableX;
            columns.forEach((c, i) => {
                doc.text(String(c), x + 4, y + 5, { width: widths[i] - 8, align: 'left' });
                x += widths[i];
            });
            y += headerH;
            ...
            data.forEach(r => {
                ensureSpace(rowH + 10);
                doc.rect(tableX, y, wFull, rowH).stroke('#000000');
                let xx = tableX;
                columns.forEach((_, i) => {
                    doc.text(String((r && r[i]) != null ? r[i] : ''), xx + 4, y + 5, { width: widths[i] - 8, align: 'left' });
                    xx += widths[i];
                });
                y += rowH;
            });
        ...
        const byRel = Array.isArray(cons.byRelacao) ? cons.byRelacao : [];
        drawTable({
            title: 'Detalhamento por relação',
            columns: ['Relação', 'Média', 'Qtd'],
            colWidths: [Math.floor(wFull * 0.5), Math.floor(wFull * 0.25), Math.floor(wFull * 0.25)],
            rows: byRel.map(r => {
                const p = r && r.pct != null ? `${(Number(r.pct) * 100).toFixed(1)}%` : '-';
                return [r.relacao || '', p, String(Number(r.count || 0))];
            })
        });

        const byCat = Array.isArray(cons.byCategoria) ? cons.byCategoria : [];
        drawTable({
            title: 'Detalhamento por categoria',
            columns: ['Categoria', 'Média'],
            colWidths: [Math.floor(wFull * 0.7), Math.floor(wFull * 0.3)],
            rows: byCat.map(r => {
                const p = r && r.pct != null ? `${(Number(r.pct) * 100).toFixed(1)}%` : '-';
                return [r.categoria || '', p];
            })
        });

        const evals = Array.isArray(payload && payload.avaliacoes) ? payload.avaliacoes : [];
        drawTable({
            title: 'Avaliações do ciclo',
            columns: ['Avaliador', 'Relação', 'Peso', 'Status', 'Resultado'],
            colWidths: [
                Math.floor(wFull * 0.32),
                Math.floor(wFull * 0.16),
                Math.floor(wFull * 0.12),
                Math.floor(wFull * 0.18),
                Math.floor(wFull * 0.22)
            ],
            rows: evals.map(a => {
                const nome = (a && (a.avaliadorNome || a.avaliadorUsername)) || '';
                const rel = (a && a.relacao) || '';
                const peso = Number.isFinite(Number(a && a.pesoFinal)) ? Number(a.pesoFinal).toFixed(2) : '-';
                const st = (a && a.status) || '';
                const p = a && a.pctCategoria != null ? `${(Number(a.pctCategoria) * 100).toFixed(1)}%` : '-';
                return [nome, rel, peso, st, p];
            })
        });
...
module.exports = {
    ...
    pdfBufferFromAvaliacaoCicloRelatorio,
